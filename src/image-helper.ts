...
import { Document, Position, Range, TextEdit, Uri, window, workspace } from 'coc.nvim';
 ...
 * @param imageFilePath
 */
export async function pasteImageFile(sourceUri: Uri | string, imageFilePath: string) {
  const finalSourceUri = typeof sourceUri === 'string' ? Uri.parse(sourceUri) : sourceUri;

  const imageFolderPath = workspace.getConfiguration('markdown-preview-enhanced').get<string>('imageFolderPath')!;
    ...
    fs.stat(destPath, (err) => {
      if (err == null) {
        ...
        if (lastDotOffset > 0) {
          ...
          imageFileName =
            imageFileName.slice(0, lastDotOffset) + uid + imageFileName.slice(lastDotOffset, imageFileName.length);
        } else {
        ...
        return window.showErrorMessage(err.toString());
      }

      void window.showInformationMessage(`Image ${imageFileName} has been copied to folder ${assetDirectoryPath}`);
      ...
      doc.applyEdits([TextEdit.insert(pos, `![${description}](${url})`)]).catch(logger.error);
    });
  });
}

function replaceHint(doc: Document, line: number, hint: string, withStr: string): boolean {
  const textLine = doc.getline(line);
  if (textLine.indexOf(hint) >= 0) {
    doc
      .applyEdits([
        TextEdit.replace(
          Range.create(Position.create(line, 0), Position.create(line, textLine.length)),
...
function setUploadedImageURL(imageFileName: string, url: string, doc: Document, hint: string, curPos: Position) {
  let description: string;
  if (imageFileName.lastIndexOf('.')) {
    description = imageFileName.slice(0, imageFileName.lastIndexOf('.'));
  } else {
 ...
 * @param imageFilePath
 */
export async function uploadImageFile(sourceUri: any, imageFilePath: string, imageUploader: string) {
